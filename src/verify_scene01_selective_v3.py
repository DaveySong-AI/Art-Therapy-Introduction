...
p=Path("output/scene-01-selective-whiteboard-v3.mp4")
d=json.loads(subprocess.check_output(["ffprobe","-v","error","-show_entries","stream=codec_type,codec_name,width,height,r_frame_rate,duration","-of","json",str(p)]))
...
assert 5.6<=float(v["duration"])<=5.9
